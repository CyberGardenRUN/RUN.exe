    ...
    def run(self):
        ...
        while self._run:
            try:
                ...
                if not m:
                    continue
                # важная правка: поддержка запятой как десятичного разделителя
...
class MplCanvas(FigureCanvasQTAgg):
    ...
    def draw_scene(self, anchors, tracker_pos):
        ...
        if all_pts:
            pts = np.array(all_pts, float)
            mn = pts.min(axis=0) - 0.5
            mx = pts.max(axis=0) + 0.5
            # если все точки совпали — зафиксируем размер осей
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.calPairs = {}        # id -> list of (d_meas, d_true)
    ...
    def solve_calibration_from_path(self):
        ...
        if T<=0:
            self.log_line("Длина записи слишком мала."); return
